				...
				{({ values, errors, touched }) => (
					<Form>
						<Container align="center" className={classes.container}>
							...
							<Paper className={classes.formControl}>
								...
								<FormControl required>
									...
									<FastField as={Select} name="chatTheme" value={values.chatTheme} align="left">
							...
							{({ insert, remove, replace }) => (
								<Container align="center" className={classes.container}>
									{values.questions && values.questions.length > 0 ? (
										values.questions.map((formField, index) => (
											<>
												<Paper className={classes.formControl}>
													...
													{(formField.answerFormat && (formField.answerFormat === "Radio" || formField.answerFormat === "Checkbox") && (
														<>
															<Button
																variant="outlined"
																		...
																		{({ insert, remove }) => (
																			<>
																				...
																				<ButtonGroup>
																					...
																					<Button className={classes.formControl} variant="text" color="secondary" onClick={() => remove(childIndex)}>
																						Remove Option Label
																					</Button>
																				</ButtonGroup>
																			</>
															...
															<>
																...
																{formField.hasOwnProperty("dateRange") && (
																	<>
																		...
																		<Field
																			as={TextField}
																			error={Date.parse(formField.dateRange[1]) - Date.parse(formField.dateRange[0]) < 0}
																			helperText={
																				Date.parse(formField.dateRange[1]) - Date.parse(formField.dateRange[0]) < 0
														...
														(formField.answerFormat === "Number" && (
															<>
																<Button
																	...
																	onClick={() => {
																		let newObject = JSON.parse(JSON.stringify(formField));
																		replace(index, Object.assign({ numberRange: [0, 0] }, newObject));
																	}}
																>
																	Confirm Answer Type{" "}
																</Button>
													...
													<ButtonGroup>
														...
														<Button className={classes.formControl} variant="contained" color="secondary" onClick={() => remove(index)}>
															Remove this Field
														</Button>
													</ButtonGroup>
												</Paper>
										...
										<Paper>
											<Button className={classes.formControl} variant="contained" color="primary" onClick={() => insert(0, initialValues)}>
												Add a Field
											</Button>
										</Paper>
									)}
						...
						<Container align="right">
							<Button className={classes.formControl} align="center" type="submit" variant="contained">
								submit
							</Button>
						</Container>
					</Form>
